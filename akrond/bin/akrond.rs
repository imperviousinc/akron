...
async fn async_main(akrond : Akron, shutdown: broadcast::Sender<()>) -> anyhow::Result<()> {
    ...
    println!("Configuration directory: {}",  project_dirs.data_dir().to_str().expect("No data dir"));
    let yuki_data_dir = project_dirs.data_dir().join("yuki");
    let spaces_data_dir = project_dirs.data_dir().join("spaces");

    let mut yuki_args = vec![
        "--data-dir",
        yuki_data_dir.to_str().expect("valid path"),
    ];
    ...
    let prune_point = format!("{}:{}",  hex::encode(checkpoint.block.hash), checkpoint.block.height);
    yuki_args.push(&prune_point);

    akrond.start(ServiceKind::Yuki, yuki_args.iter().map(|s| s.to_string()).collect()).await?;
    akrond.start(ServiceKind::Spaces, spaces_args.iter().map(|s| s.to_string()).collect()).await?;
